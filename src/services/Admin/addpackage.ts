...
export const addNewPackage = async (newPackageData:any) => {
  ...
  try {
    ...
    // return insertedServices;
      ...
      const { name,description, category_id,parent_id,price } = newPackageData;

      const existingService = await PackageModel.findOne({category_id:newPackageData.category_id });
